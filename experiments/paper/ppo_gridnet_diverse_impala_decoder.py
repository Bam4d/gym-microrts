...
class DecodeSeq(nn.Module):
    def __init__(self, in_channels, out_shape):
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(in_channels, out_shape[0], 3, stride=2, padding=1, output_padding=1),
            nn.ReLU(),
        )
...
class Agent(nn.Module):
    def __init__(self, mapsize=16 * 16):
        ...
        shape = (c, h, w)
        decoder_shape = (78, h, w)
        conv_seqs = []
        deconv_seqs = []
        for out_channels in [16, 32, 32]:
            deconv_seqs.append(DecodeSeq(out_channels, decoder_shape))
        ...
        encoding_size = np.prod(shape)

        self.actor = nn.Sequential(*deconv_seqs)

        self.critic = nn.Sequential(
            nn.Flatten(),
            layer_init(nn.Linear(encoding_size, 64), std=1),
            nn.ReLU(),
